...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def keyPressEvent(self, event):
        # Add shortcut for F key to fit window
        if event.key() == QtCore.Qt.Key_F:
            self.setFitWindow(True)
            return
            
        # Add brightness and contrast adjustment with keyboard
        if hasattr(self, 'imageData') and self.imageData is not None:
            # Check if B or C is being held (for brightness or contrast)
            if event.key() == QtCore.Qt.Key_B:
                self.brightness_key_pressed = True
                return
            elif event.key() == QtCore.Qt.Key_C:
                self.contrast_key_pressed = True
                return
                
            # Check for arrow keys to adjust values
            if hasattr(self, 'brightness_key_pressed') and self.brightness_key_pressed:
            ...
            if hasattr(self, 'contrast_key_pressed') and self.contrast_key_pressed:
                if event.key() == QtCore.Qt.Key_Up or event.key() == QtCore.Qt.Key_Down:
                    # Get current values or use defaults
                    brightness, contrast = self.brightnessContrast_values.get(
                        self.filename, (50, 50)
    ...
    def keyReleaseEvent(self, event):
        # Handle key releases for B and C
        if event.key() == QtCore.Qt.Key_B:
            self.brightness_key_pressed = False
        elif event.key() == QtCore.Qt.Key_C:
            self.contrast_key_pressed = False
